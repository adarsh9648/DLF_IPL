...
$(window).on("load", function () {
  ...
  if (team) {
    ...
    arrFilter.map((players) => {
      ...
      console.log(playerFun, "playerFun");
      playerCards +=
        '<div class="col-md-6 col-lg-3 d-flex align-items-stretch mb-5 mb-lg-0 detailCard" data-aos="fade-up" data-aos-c="200" id="detailCard' +
        players.id +
        '" ' +
        playerFun +
        '><div class="icon-box"><div class="icon"><img src="assets/img/' +
        players.description +
        '.png" class="player-icon" /></div><h4 class="title">Full Name - ' +
        players.playerName +
        '</h4><p class="description">Team - ' +
        team +
        '</p><p class="description">Price - ' +
        players.price +
        '</p><p class="description">Player Status - ' +
        playerStaus +
        '</p><p class="description">Role - ' +
        players.description +
        "</p></div></div>";
    });
